  ...
  if (stage === "Submitted") {
    return (
      <svg viewBox="0 0 24 24" fill="none" className="h-4 w-4">
        <path d="M6.5 12.5l3 3L17.5 8" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    );
  }
...
function buildSeedGallery(project: ManagedProject): GalleryPhoto[] {
  return project.assets.slice(0, 3).map((asset, index) => ({
    id: index + 1,
    name: asset.label,
    url: asset.previewImage,
    fromUser: false,
  }));
}

export function SubmitContentWorkspace({
  selectedProjectId,
}: {
  ...
  const [galleryPhotos, setGalleryPhotos] = useState<GalleryPhoto[]>(
    selectedProject ? buildSeedGallery(selectedProject) : []
  );
      ...
      <div className="mx-auto w-full max-w-[1080px] space-y-6">
        <section className="reveal-enter text-center">
          ...
          <p className="mt-2 text-base text-[#7c879b]">
            Choose a current campaign first, then upload the deliverables for that project.
          </p>
        </section>

        <section className="reveal-enter grid gap-5 md:grid-cols-2 xl:grid-cols-3" style={{ animationDelay: "80ms" }}>
          {activeProjects.map((project, index) => (
            <Link
              key={project.id}
              href={`/submit-content?project=${project.id}`}
              className="reveal-enter group overflow-hidden rounded-3xl bg-white shadow-[0_10px_28px_rgba(18,31,53,0.08)] transition hover:-translate-y-1 hover:shadow-[0_18px_36px_rgba(18,31,53,0.12)]"
              style={{ animationDelay: `${120 + index * 40}ms` }}
            >
              <div className="relative h-52 bg-cover bg-center" style={{ backgroundImage: `url('${project.assets[0]?.previewImage ?? ""}')` }}>
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.14em] text-white/75">{project.brand}</p>
                  <h2 className="mt-2 text-2xl font-semibold text-white">{project.title}</h2>
                </div>
              </div>

              <div className="space-y-4 p-5">
                <p className="text-sm leading-6 text-[#5f6b81]">{project.brief}</p>

                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div className="rounded-2xl bg-[#f8fafc] p-3">
                    <p className="text-xs text-[#8b97ab]">Deadline</p>
                    <p className="mt-1 font-semibold text-[#2f3747]">{project.due}</p>
                  </div>
                  <div className="rounded-2xl bg-[#f8fafc] p-3">
                    <p className="text-xs text-[#8b97ab]">Payout</p>
                    <p className="mt-1 font-semibold text-[#2f3747]">{project.payout}</p>
                  </div>
                </div>

                <div className="flex flex-wrap gap-2">
                  <span className="rounded-full bg-[#eef3ff] px-3 py-1 text-xs font-semibold text-[#4f6fd3]">
                    {project.uploadedVideos}/{project.requiredVideos} Video
                  </span>
                  <span className="rounded-full bg-[#eef3ff] px-3 py-1 text-xs font-semibold text-[#4f6fd3]">
                    {project.uploadedPhotos}/{project.requiredPhotos} Photos
                  </span>
                  <span className="rounded-full bg-[#eef3ff] px-3 py-1 text-xs font-semibold text-[#4f6fd3]">
                    {project.uploadedStories}/{project.requiredStories} Stories
                  </span>
                </div>

                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium text-[#7c879b]">Open submission workspace</p>
                  <span className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-r from-[#de8b34] to-[#e36d58] text-white">
                    <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5">
                      <path d="M8 6l8 6-8 6" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </span>
                </div>
              </div>
  ...
  const requiredVideoCount = selectedProject.requiredVideos;
  const requiredPhotoCount = selectedProject.requiredPhotos;
  ...
  const onAddVideos = (files: FileList | null) => {
    ...
    const validVideos = Array.from(files).filter((file) => /\.(mp4|mov)$/i.test(file.name));
  ...
  return (
    <div className="mx-auto w-full max-w-[1080px] space-y-6">
      <section className="reveal-enter flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-4xl font-semibold text-[#2f3747]">Submit Content</h1>
          <p className="mt-2 text-base text-[#7c879b]">Upload your deliverables for {selectedProject.title}.</p>
        </div>
        <Link
          href="/submit-content"
      ...
      <section className="reveal-enter grid gap-5 lg:grid-cols-[2fr_1fr]" style={{ animationDelay: "140ms" }}>
        <div className="space-y-5">
          ...
          >
            ...
            />
            <button type="button" onClick={() => videoInputRef.current?.click()} className="w-full">
              <span className="mx-auto inline-flex h-12 w-12 items-center justify-center rounded-full bg-white text-[#de8b34] shadow-sm">
                <svg viewBox="0 0 24 24" fill="none" className="h-6 w-6">
                  <path d="M12 15V4M8 8l4-4 4 4M5 15v3a2 2 0 002 2h10a2 2 0 002-2v-3" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </span>
              ...
              {uploadedVideos.map((video) => (
                <div key={video.id} className="hover-lift rounded-2xl bg-white p-3 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
                  <div className="flex items-center gap-3">
                    ...
                    <button type="button" onClick={() => onRemoveVideo(video.id)} className="px-2 text-[#9aa5b7] transition hover:text-[#66758e]" aria-label={`Remove ${video.name}`}>
                      x
                    </button>
                  </div>
                </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
              {galleryPhotos.map((photo) => (
                <div key={photo.id} className="group hover-lift relative h-28 overflow-hidden rounded-2xl">
                  <div className="h-full w-full bg-cover bg-center" style={{ backgroundImage: `url('${photo.url}')` }} role="img" aria-label={photo.name} />
                  {photo.fromUser && (
                    <button
                      type="button"
              ...
              <input ref={photoInputRef} type="file" accept="image/*" multiple className="hidden" onChange={(event) => onAddPhotos(event.target.files)} />
              <button
                type="button"
              ...
              >
                <span className="mx-auto flex h-full w-full flex-col items-center justify-center gap-2 text-xs font-medium">
                  <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5">
                    ...
                    <path d="M8 13l2.2-2.2a1 1 0 011.4 0L16 15M14 11h.01M12 9v6M9 12h6" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                  Add Photo
                </span>
              </button>
        ...
        <aside className="space-y-4">
          <article className="reveal-enter hover-lift rounded-3xl bg-white p-5 shadow-[0_2px_8px_rgba(0,0,0,0.05)]" style={{ animationDelay: "220ms" }}>
            ...
            <div className="mt-4 border-b border-[#edf1f6] pb-4">
              <p className="text-2xl font-semibold text-[#2f3747]">{selectedProject.title}</p>
              <p className="mt-1 text-sm text-[#7c879b]">{selectedProject.brand}</p>
            </div>
            <div className="mt-4 space-y-3">
              <div>
                <p className="text-sm text-[#7c879b]">Deliverables</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  <span className="rounded-full bg-[#eaf1ff] px-2.5 py-1 text-xs font-semibold text-[#4f6fd3]">{uploadedVideos.length} / {requiredVideoCount} VIDEO</span>
                  <span className="rounded-full bg-[#eaf1ff] px-2.5 py-1 text-xs font-semibold text-[#4f6fd3]">{uploadedPhotoCount} / {requiredPhotoCount} PHOTOS</span>
                  <span className="rounded-full bg-[#eaf1ff] px-2.5 py-1 text-xs font-semibold text-[#4f6fd3]">{selectedProject.uploadedStories} / {selectedProject.requiredStories} STORIES</span>
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="mt-1 text-base font-semibold text-[#dc4869]">{selectedProject.due}</p>
              </div>
            </div>
          </article>
          ...
          <article className="reveal-enter hover-lift rounded-3xl bg-white p-5 shadow-[0_2px_8px_rgba(0,0,0,0.05)]" style={{ animationDelay: "280ms" }}>
            ...
            <div className="mt-4 space-y-3">
              <label className="block">
                ...
                <textarea rows={3} value={caption} onChange={(event) => setCaption(event.target.value)} placeholder="Write a caption for your content..." className="w-full rounded-xl border border-[#d6dce7] bg-[#fafbfd] p-3 text-sm text-[#3d4860] outline-none focus:border-[#b8c5dd]" />
              </label>
              <label className="block">
                <span className="mb-1 block text-sm font-medium text-[#48556d]">Notes to Brand</span>
                <textarea rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Any additional notes..." className="w-full rounded-xl border border-[#d6dce7] bg-[#fafbfd] p-3 text-sm text-[#3d4860] outline-none focus:border-[#b8c5dd]" />
              </label>
            </div>
          </article>
